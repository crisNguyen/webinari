...
class StripeService
  ...
  def find_or_create_customer(customer)
    return Stripe::Customer.retrieve(customer.stripe_customer_id) if customer.stripe_customer_id.present?
  ...
  def create_stripe_charge(amount_to_be_paid, stripe_customer_id, card_id, workshop)
    Stripe::Charge.create({
      ...
      description: "Amount $#{amount_to_be_paid} charge for #{workshop.name}"
    })
  end
end